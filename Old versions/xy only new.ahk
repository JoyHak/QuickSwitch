...
Loop
{
  ...
  If DialogType										;	This is a supported dialog
  {
    ...
    If DialogAction 								;	======= AutoSwitch ==
    {
      FolderPath := Get_Zfolder(WinID)
      FeedDialog%DialogType%(WinID, FolderPath)

    }
    Else If !DialogAction							;	======= Never here ==
    {
      If ShouldOpen()								;	======= Show Menu ==
      {
        ShowFoldersMenu() 	; only show with AutoOpenMenu = 1
      }
    }
    Else If ShouldOpen()								;	======= Show Menu ==
    {
	  ShowFoldersMenu() 	; only show with hotkey ctrl-q, or AutoOpenMenu = 1
    }
    ...
    ; Activate the CTR-Q hotkey. When pressed, start the ShowFoldersMenu routine
...
;_____________________________________________________________________________
;
SetDefaultValues(_overwrite := false)
;_____________________________________________________________________________
;
{
  ...
  If _overwrite
  {
	...
	IniWrite, 	%PathSeparator%, 			%INI%, 	Menu, 		PathSeparator
    IniWrite, 	%ShortNameIndicator%, 		%INI%, 	Menu, 		ShortNameIndicator
...
;_____________________________________________________________________________
{
  ...
  ; SHORT PATH	
  Gui, 	Add, 	Text, 		x30 y+20,									Path separator			
  Gui, 	Add, 	Edit, 		x230 yp-4 w60 	vSeparatorGUI, 				%PathSeparator%
  
  Gui, 	Add, 	Checkbox, 	x30  w200		vShortPathGUI gShortPath, 	Show short path											
  Gui, 	Add, 	Edit, 		x230 yp-4 w60 	vIndicatorGUI, 				%ShortNameIndicator%
  ...
  Gui, 	Add, 	Edit, 		x230 yp-4 w60 	vMenuColorGUI, 				%MenuColor%
		
  Gui, 	Add, 	Text, 		x30, 										&Dialogs background color (HEX)
  Gui, 	Add, 	Edit, 		x230 yp-4 w60 	vGuiColorGUI, 				%GuiColor%
...
ShortPath()
{
  ...
  If (ShortPathGUI) 
  {
	GuiControl, , ShortPathGUI, Show short path, indicate as:
	GuiControl, Show, IndicatorGUI
  } 
  Else 
  {
	GuiControl, , ShortPathGUI, Show short path
	GuiControl, Hide, IndicatorGUI
  }
return
...
;
  ...
  Gui, Submit  
  FolderNumGUI     	  := FolderNumGUI - 1
  
  ; check valid values and write them, otherwise write default
  OpenMenu     		:= 	ValidateWriteInteger(OpenMenuGUI     , 	OpenMenu     , 	  "AlwaysOpenMenu")
  ShortPath     		:= 	ValidateWriteInteger(ShortPathGUI    , 	ShortPath     , 	  "ShortPath")
  ReDisplayMenu		:= 	ValidateWriteInteger(ReDisplayMenuGUI, 	ReDisplayMenu, 	  "ReDisplayMenu")
  FolderNum    		:= 	ValidateWriteInteger(FolderNumGUI    , 	FolderNum    , 	  "ShowFolderNumbers")
  
  PathSeparator		:= 	ValidateWriteString(SeparatorGUI	 , 	PathSeparator, 	  "PathSeparator")
  ShortNameIndicator 	:=  ValidateWriteString(IndicatorGUI	 ,  ShortNameIndicator,  "ShortNameIndicator")
  
  GuiColor     		:= 	ValidateWriteColor(GuiColorGUI       , 	GuiColor     , 	  "GuiBGColor")
  MenuColor    		:= 	ValidateWriteColor(MenuColorGUI      , 	MenuColor    , 	  "MenuBGColor")
